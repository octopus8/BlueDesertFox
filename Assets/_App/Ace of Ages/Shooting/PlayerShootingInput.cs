...
public class PlayerShootingInput : MonoBehaviour
{
    ...
    /// <summary>Starts the coroutine that waits for the <see cref="PlayerShip"/> entity to be baked before binding input.</summary>
    void Start()
    {
    ...
    /// <summary>Re-enables the <c>Fire</c> input action when the component is enabled.</summary>
    void OnEnable()
    {
        if (_fireAction != null)
        {
    ...
    /// <summary>Disables the <c>Fire</c> input action when the component is disabled to prevent phantom input events.</summary>
    void OnDisable()
    {
        if (_fireAction != null)
        {
