...
```
Main Scene
├── AceOfAges.cs          (entry point — triggers enemy spawns)
├── PlayerMover.cs        (VR locomotion MonoBehaviour)
├── PlayerShootingInput.cs (Input System → ECS doShoot flag)
│
└── Entities Subscene (ECS World)
    ├── Terrain systems       ← infinite procedural terrain
    ├── Static object systems ← trees, turrets, decorations
    ├── Enemy spawner         ← bowling-pin formations on splines
    ├── Shooting systems      ← bullet pool, collision, VFX
    ├── Effects               ← dirt explosion pool
    ├── TransformFollower     ← ECS entities → GameObject bridge
    └── Splines               ← BlobAsset spline data
```
...
├── TransformFollowerInitSystem
├── BulletPoolSystem
├── DirtExplosionPoolSystem
└── StaticObjectLODMeshInfoInitSystem

SimulationSystemGroup
├── ResetEventsSystem              (resets doSpawn, doShoot)
├── EnemySpawnerSystem             (before ResetEvents)
├── FormationMovementSystem        (before SplineFollower)
├── PlayerScrollVelocitySystem     (or ConstantScrollVelocitySystem)
├── ScrollTerrainSystem
├── TileSpawningSystem
...
├── StaticObjectSpatialChunkingSystem
├── TreeLODUpdateSystem (StaticObjectLODUpdateSystem.cs)
├── BulletShooterSystem
├── BulletLifecycleSystem
├── FormationCleanupSystem         (LateSimulationSystemGroup)
└── StaticObjectLinkedRendererStripSystem (after EndSimulation ECB)

TransformSystemGroup
├── PlayerTargetVelocityEstimateSystem
├── TransformFollowerSystemOptimized
├── StaticObjectPositionUpdateSystem (objectPositionUpdateSystem.cs)
├── TurretAimingSystem
├── TurretBarrelSystem
└── TurretShooterSystem

FixedStepSimulationSystemGroup
├── BulletTerrainScrollVelocitySystem
└── BulletCollisionSystem

PresentationSystemGroup
├── TerrainRenderingSystem
└── DirtExplosionPlaySystem
```
