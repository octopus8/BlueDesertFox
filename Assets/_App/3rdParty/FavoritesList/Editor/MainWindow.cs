    ...
    public class MainWindow : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            using (var scrollView = new GUILayout.ScrollViewScope(scrollPos, false, false))
            {
                ...
                foreach (var reference in list.References.Where(reference =>
                             !reference.GetProvidedData().SceneData.IsScene))
                {
                ...
                foreach (var reference in from reference in list.References
                         where reference.GetProvidedData().SceneData.IsScene
                         where SearchUtils.IsSearched(reference, searchText)
                         where DrawElement(reference)
                         select reference)
                {
                    list.RemoveReference(reference);
                    break;
        ...
        private bool DrawElement(ObjectReference reference)
        {
            ...
            {
#if UNITY_5_4_OR_NEWER
                GUILayout.Label(drawData.Icon, GUILayout.Width(ICON_SIZE), GUILayout.Height(ICON_SIZE),
                    GUILayout.MaxWidth(ICON_SIZE), GUILayout.MaxHeight(ICON_SIZE));
#else
            buttonWidth += ICON_SIZE + 10;
#endif

                if (GUILayout.Button(drawData.Name, GUILayout.MaxWidth(buttonWidth), GUILayout.Width(buttonWidth),
                ...
                if (sceneData.IsScene)
                {
                    ...
                    {
                        if (GUILayout.Button("Load", GUILayout.MaxWidth(buttonWidth * 2 + 2.5f),
                                GUILayout.Width(buttonWidth * 2 + 2.5f), GUILayout.ExpandWidth(false)))
                        {
                    ...
                    {
                        if (GUILayout.Button("Load", GUILayout.MaxWidth(buttonWidth), GUILayout.Width(buttonWidth),
                                GUILayout.ExpandWidth(false)))
                        {
                        ...
                        if (GUILayout.Button("Play", GUILayout.MaxWidth(buttonWidth), GUILayout.Width(buttonWidth),
                                GUILayout.ExpandWidth(false)))
                        {
                            if (EditorSceneManager.SaveCurrentModifiedScenesIfUserWantsTo())
                                EditorSceneManager.OpenScene(AssetDatabase.GetAssetOrScenePath(scene),
                ...
                if (GUILayout.Button(CLOSE_ICON, GUIStyle.none, GUILayout.Width(ICON_SIZE),
